...
export class TournamentPage {
  ...
  inscriptionConfirmation(car: Car) {
    this.alertService.presentAlertWithButtons(
      this.translatePipe.transform('tournament.inscriptionConfirmationTitle'),
    ...
    this.inscriptionService.create(inscription).subscribe({
      next: (response) => {
        ...
        this.alertService.presentAlert(
          this.translatePipe.transform('tournament.inscriptionOkTitle'),
          `${this.translatePipe.transform('tournament.inscriptionOkMessage')} ${car.brand.name
          } ${car.model}`
        );
      },
      error: (err) => {
  ...
  confirmDeleteInscription(car: Car) {
    this.alertService.presentAlertWithButtons(
      this.translatePipe.transform('tournament.inscriptionDeleteConfirmationTitle'),
      this.translatePipe.transform('tournament.inscriptionDeleteConfirmationMessage'),
      [
        {
          text: 'Cancelar',
          role: 'cancel',
    ...
    this.inscriptionService
      ...
      .subscribe({
        next: () => {
          ...
          this.alertService.presentAlert(
            this.translatePipe.transform('tournament.inscriptionDeleteOkTitle'),
            this.translatePipe.transform('tournament.inscriptionDeleteOkMessage')
          );
          this.analyticsService.logEvent(
